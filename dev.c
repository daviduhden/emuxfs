...
emuxfs_dev_state_write_fd(int fd, struct emuxfs_dev_state *state)
{
	...
	EMUXFS_TRACE("state_write fd=%d seq=%llu mounted=%llu working=%llu "
		     "restoring=%llu degraded=%llu",
	    fd, (unsigned long long)state->seq,
...
emuxfs_dev_init(dind dev_index)
{
	...
	dev->root_fd = dev->muxfs_fd = dev->state_fd = dev->meta_fd =
	    dev->assign_fd = dev->lfile_fd = -1;
}
...
emuxfs_dev_open(dind dev_index, int force, int readonly)
{
	...
	root_fd = muxfs_fd = conf_fd = state_fd = meta_fd = assign_fd =
	    lfile_fd = -1;
	...
	if ((root_fd = open(
		 dev->root_path, O_RDONLY | O_DIRECTORY | O_CLOEXEC)) == -1)
		goto fail;
	...
	if (rc != EMUXFS_CONF_OK) {
		if (rc == EMUXFS_CONF_EBADVERSION)
			dprintf(2,
			    "Error: %s: unsupported on-disk format "
			    "version.\n",
			    dev->root_path);
		goto fail;
	}
	...
	file_flags = readonly ? (O_RDONLY | O_NOFOLLOW | O_CLOEXEC) :
	...
	if (rc == 2) {
		...
		if (!force) {
			dprintf(2,
			    "Error: %s: structurally invalid state.db "
			    "(impossible field combination).\n",
			    dev->root_path);
			goto fail;
		}
		dprintf(2,
		    "Warning: %s: structurally invalid state.db; "
...
emuxfs_meta_read(struct emuxfs_meta *meta, dind dev_index, uint64_t ino)
{
	...
	if (ino > ((uint64_t)INT64_MAX / msz)) {
		EMUXFS_TRACE("meta_read: ino too large dev=%lu ino=%llu "
			     "msz=%zu\n",
		    (unsigned long)dev_index, (unsigned long long)ino, msz);
		return 1;
	}

	memset(&disk_meta, 0, sizeof(disk_meta));
	if (emuxfs_pread_exact(
		dev->meta_fd, &disk_meta, msz, (off_t)(ino * msz))) {
		EMUXFS_TRACE("meta_read: pread dev=%lu ino=%llu off=%lld "
			     "msz=%zu\n",
		    (unsigned long)dev_index, (unsigned long long)ino,
		    (long long)(ino * msz), msz);
		return 1;
	}
...
EMUXFS int
emuxfs_meta_write_fd(
    int fd, const struct emuxfs_meta *meta, uint64_t ino, size_t msz)
{
	EMUXFS_TRACE("enter");
	struct emuxfs_meta disk_meta;
...
emuxfs_assign_read(struct emuxfs_assign *assign, dind dev_index, uint64_t eno)
{
	...
	if (emuxfs_pread_exact(
		dev->assign_fd, &disk_assign, asz, (off_t)(eno * asz)))
		return 1;
...
EMUXFS int
emuxfs_assign_write(
    const struct emuxfs_assign *assign, dind dev_index, uint64_t eno)
{
	EMUXFS_TRACE("enter");
	struct emuxfs_dev *dev;
